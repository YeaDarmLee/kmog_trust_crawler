# -*- coding: utf-8 -*-
# crawl_hanatrust.py
# 하나자산신탁 공매/매각 리스트 → Google Sheets append
# - 교보 로직과 동일한 상태관리/중복 규칙/재시작 안전/페이지 내 역순 처리 유지
# - 목록에서 번호(없음→빈값), 제목(위치), 게시일, 상세 URL 파싱
...
# ────────── 크롤링 설정 ──────────
BASE = "https://www.hanatrust.com"
LIST_PATH = "/ko/publicSale/goodsForPublicSale"
TRUST_NAME = "하나자산신탁"
...
RESUME = True
STATE_FILE = "output/.hanatrust_state.json"
...
def setup_logging():
  ...
  logfile = f"logs/hanatrust_sheets_{ts}.log"
  logging.basicConfig(
    level=logging.INFO,
...
def parse_list_page(html: str) -> List[Dict[str, str]]:
  """
  <ul class="auction-sale__list">
    <li class="auction-sale__item ...">
      <a class="auction-sale__link" href="/ko/publicSale/goodsForPublicSale/view?seq=...&page=...">
        <div class="auction-sale__description">
          <span class="auction-sale__purpose">토지</span>
          <p class="auction-sale__position">제목(주소/물건)</p>
          <div class="auction-sale__others">
            <span class="auction-sale__date">YYYY.MM.DD</span>
          </div>
        </div>
        <span class="auction-sale__status">완료/중지/취소/진행중...</span>
      </a>
      (감정평가서 링크는 javascript:alert 이라 무시)
    </li>
  """
  soup = BeautifulSoup(html, "lxml")
  items: List[Dict[str, str]] = []

  for li in soup.select("ul.auction-sale__list li.auction-sale__item"):
    a = li.select_one("a.auction-sale__link[href]")
    href = a.get("href", "") if a else ""
    url = urljoin(BASE, href) if href else ""

    # 제목(주소 위주)
    title_el = li.select_one(".auction-sale__position")
    title = title_el.get_text(" ", strip=True) if title_el else ""

    # 게시일
    date_el = li.select_one(".auction-sale__date")
    post_date = _normalize_date(date_el.get_text(" ", strip=True) if date_el else "")

    # 번호: 페이지 내 순번 요소가 없으므로 공란 처리(스키마 유지용)
    no = ""
