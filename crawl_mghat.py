...
def parse_list_items(html: str, base: str) -> List[Dict[str, str]]:
  ...
  if not rows:
    ...
    for a in links:
      ...
      items.append({"trust_name": "무궁화신탁", "title": title, "post_date": date_str, "url": url})
    return items
...
def _open_csv_writer(path: str):
  ...
  w = csv.DictWriter(f, fieldnames=["trust_name", "title", "post_date", "url"])
  if not file_exists:
    w.writeheader()
  return f, w
...
def main():
  ...
  try:
    for page in range(start_page, END_PAGE - (1 if STEP < 0 else -1), STEP):
      ...
      # seen_urls가 커질 수 있으니 주기적으로만 저장해도 됨. 여기서는 매 페이지 저장.
