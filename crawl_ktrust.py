# -*- coding: utf-8 -*-
# crawl_ktrust.py
# 코리아신탁 공매/매각 리스트 → Google Sheets append
...
RESUME = True
STATE_FILE = "output/.ktrust_state.json"
...
def setup_logging():
  ...
  logfile = f"logs/ktrust_sheets_{ts}.log"
  logging.basicConfig(
    level=logging.INFO,
...
# ────────── 리스트 파서 (코리아신탁 전용) ──────────
def parse_list_page(html: str, list_url: str) -> List[Dict[str, str]]:
  """
  구조 예시:
  <tbody>
    <tr>
      <td class="pc">12</td>
      <td><a href="?pages=thing&subs=thing02&m=view&idx=13&page=66">제목</a></td>
      <td class="state"><span class="d">중지</span></td>
  ...
  for tr in soup.select("tbody > tr"):
    ...
    # 번호
    no_el = tr.select_one("td.pc") or tds[0]
    no_m = re.search(r"\d+", no_el.get_text(strip=True) if no_el else "")
    no = no_m.group(0) if no_m else ""

    # 제목/링크
    a = tr.select_one("td a[href]")
    title = a.get_text(" ", strip=True) if a else ""
    href = a.get("href", "") if a else ""
    # href가 '?pages=...' 형태라서 list_url 기준으로 join
    url = urljoin(list_url, href) if href else ""

    # 게시일
    date_el = tr.select_one("td.date")
    post_date = _normalize_date(date_el.get_text(strip=True) if date_el else tds[-1].get_text(strip=True))
...
def main():
  ...
  try:
    ...
    for page in range(start_page, rng_end_inclusive, STEP):
      ...
      items = parse_list_page(html, r.url)
