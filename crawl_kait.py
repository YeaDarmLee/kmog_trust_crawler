# -*- coding: utf-8 -*-
# crawl_kait.py
# 한국자산신탁 공매/매각 리스트 → Google Sheets append
# - 교보 크롤러와 동일한 상태관리/중복 규칙/재시작 안전/페이지 내 역순 처리 유지
# - 목록에서 번호/제목/게시일/상세 URL 파싱
...
RESUME = True
STATE_FILE = "output/.kait_state.json"
...
def setup_logging():
  ...
  logfile = f"logs/kait_sheets_{ts}.log"
  logging.basicConfig(
    level=logging.INFO,
...
def parse_list_page(html: str) -> List[Dict[str, str]]:
  ...
  <tbody>
    <tr>
      ...
      <td class="kboard-list-class">유찰/낙찰...</td>
      <td class="kboard-list-title">
        <a href="/sale-info/?pageid=14&mod=document&uid=6984">
          <div class="kboard-default-cut-strings">[지역 주소 …] 공매공고</div>
        </a>
        ... (모바일 영역에도 날짜가 존재)
      </td>
      <td class="kboard-list-user">관리자</td>
      <td class="kboard-list-date">2018.06.12</td>
      ...
    </tr>
  """
  ...
  for tr in soup.select("tbody > tr"):
    # 번호
    no_el = tr.select_one("td.kboard-list-uid")
    no = ""
    if no_el:
    ...
    # 제목/링크
    a = tr.select_one("td.kboard-list-title a[href]")
    title = ""
    href = ""
    if a:
      title_el = a.select_one(".kboard-default-cut-strings")
      title = (title_el.get_text(" ", strip=True) if title_el
               else a.get_text(" ", strip=True))
      href = a.get("href", "")
    url = urljoin(BASE, href) if href else ""

    # 게시일: 우선 우측 날짜 셀, 없으면 모바일 영역의 날짜
    date_el = tr.select_one("td.kboard-list-date")
    post_date = _normalize_date(date_el.get_text(" ", strip=True) if date_el else "")
    if not post_date:
      mob_date = tr.select_one(".kboard-mobile1111-contents .kboard-date")
      post_date = _normalize_date(mob_date.get_text(" ", strip=True) if mob_date else "")
