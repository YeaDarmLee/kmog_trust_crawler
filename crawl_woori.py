# -*- coding: utf-8 -*-
# crawl_woori.py
# 우리자산신탁 공매/매각 리스트 → Google Sheets append
...
# ────────── 크롤링 설정 ──────────
BASE = "https://www.wooriat.com"
LIST_PATH = "/item/vendue/list"
TRUST_NAME = "우리자산신탁"

DATE_RE = re.compile(r"\d{4}\.\s*\d{2}\.\s*\d{2}")
DELAY_SEC = 1.0

# 페이지 진행 방향(사이트에 맞게 조정 가능)
START_PAGE = 415
END_PAGE   = 1
STEP       = -1

RESUME = True
STATE_FILE = "output/.woori_state.json"
...
def setup_logging():
  ...
  logfile = f"logs/woori_sheets_{ts}.log"
  logging.basicConfig(
    level=logging.INFO,
...
def _normalize_duplicate_numbering(ws):
  """같은 주소가 2건 이상인 그룹만 '중복1..N'로 채우고, 1건이면 공란 유지."""
...
def parse_list_page(html: str) -> List[Dict[str, str]]:
  ...
  <tbody>
    <tr>
      <td>4150</td>
      <td class="state"><span class="ing">진행</span></td>
      <td class="tit3"><a href="/item/vendue/detail/24392">제목</a></td>
      <td>...</td>
      <td>2025. 09. 16</td>
    </tr>
    ...
  ...
  for tr in soup.select("tbody > tr"):
    ...
    # 번호
    no = re.search(r"\d+", tds[0].get_text(strip=True) if tds[0] else "")
    no = no.group(0) if no else ""

    # 제목/링크
    a = tr.select_one("td.tit3 a[href]")
    ...
    post_date = _normalize_date(date_text)
...
def main():
  ...
  try:
    ...
    for page in range(start_page, rng_end_inclusive, STEP):
      params = {"page": page, "sortStatus": "", "searchWord": ""}
